...
<head>
	...
	<script type="text/javascript">
        ...
        function add() {
			window.location.href="${ctx}/sys/user/performance/form?user.id=${performance.user.id}";
        }
        function save(){
            if ($('#inputForm').valid()) {
                $('#inputForm').submit();
...
<ul class="nav nav-tabs">
	...
	<li class="active"><a href="${ctx}/sys/user/performance/list?user.id=${user.id}">绩效信息</a></li>
...
<div class="btn-toolbar ">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${list}" var="performance">
			<tr>
				<%--<td>--%>
					<%--<fmt:formatDate value="${performance.year}" pattern="yyyy-MM"/>--%>
				<%--</td>--%>
					<td>${performance.year}</td>
				<td>${performance.score}</td>
				<td>${fns:getDictLabel(performance.level, 'sys_user_perf_level', '无')}</td>
				<td>${performance.remark}</td>
				<shiro:hasPermission name="sys:user:edit">
				<td>
					<a href="${ctx}/sys/user/performance/form?id=${performance.id}">编辑</a>
					<a href="${ctx}/sys/user/performance/delete?id=${performance.id}" onclick="return confirmx('确认要删除该员工绩效吗？', this.href)">删除</a>
...
<!-- 模态框（Modal） -->
<form:form id="inputForm" modelAttribute="performance" action="${ctx}/sys/user/performance/save" method="post" class="form-horizontal">
<form:hidden path="id"/>
	...
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title" id="myModalLabel">员工绩效${not empty performance.id?'修改':'添加'}</h4>
			</div>
			<div class="modal-body">

				   <div class="control-group">
					    <label >&nbsp;&nbsp;年&nbsp;&nbsp;&nbsp;&nbsp;度&nbsp;：</label>
					    <input id="year" name="year" type="text" readonly="readonly" maxlength="20" class="input-small Wdate "
					...
					<div class="control-group">
						<label>&nbsp;&nbsp;分&nbsp;&nbsp;&nbsp;&nbsp;数&nbsp;：</label>
						<form:input path="score" htmlEscape="false" maxlength="200" onkeyup="onlyNum(this)"  class="input-small"  required="true" />
						<span class="help-inline"><font color="red">*</font> </span>
					</div>
					<div class="control-group">
						<label>&nbsp;&nbsp;等&nbsp;&nbsp;&nbsp;&nbsp;级&nbsp;：</label>
						<form:select path="level" class="input-small ">
							<form:options items="${fns:getDictList('sys_user_perf_level')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
						</form:select>
					</div>
				    <div class="control-group">
					     <label>&nbsp;&nbsp;备&nbsp;&nbsp;&nbsp;&nbsp;注&nbsp;：</label>
