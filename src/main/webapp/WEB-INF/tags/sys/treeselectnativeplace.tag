			...
			ajaxData:{selectIds: $("#${id}Id").val()},buttons:{"确定":"ok", ${allowClear?"\"清除\":\"clear\", ":""}"关闭":true}, submit:function(v, h, f){
				if (v=="ok"){
					...
					for(var i=0; i<nodes.length; i++) {//<c:if test="${checked && notAllowSelectParent}">
						...
						ids.push(nodes[i].id);
						if(nodes[i].parent.name!=null&&nodes[i].parent.name!=""){
                            switch(nodes[i].name)
                            {
                                case "北京":
                                case "天津":
                                ...
                                case "香港":
                                case "澳门":
                                case "台湾":names.push(nodes[i].name);break;
                                default:names.push(nodes[i].parent.name+nodes[i].name);
                            }
                            //names.push(nodes[i].parent.name+nodes[i].name);
						}else{
